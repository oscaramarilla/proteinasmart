...
export default function FloatingCart() {
  ...
  function pedirPorWhatsApp() {
    window.open(generarCheckoutWhatsApp(items, total), '_blank', 'noopener,noreferrer');
    limpiarCarrito();
  }
    ...
    <aside className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-700 bg-slate-950 px-4 py-3 text-white shadow-[0_-8px_30px_rgba(15,23,42,0.18)] sm:px-6">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
        ...
        <button
          ...
          className="rounded-xl bg-[#25D366] px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-[#20bd5a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 active:scale-[0.98] sm:px-6"
        >
          Pedir por WhatsApp
        </button>
      </div>
